...
```{r tourinit, fig.cap="A 2D guided tour of the five-dimensional space spanned by the real-valued variables of the crabs dataset. The tour starts from a projection identified by @Mass that almost separates the four sex-species classes and explores the views in the neighbourhood."}
# Replace with crosstalk + plotly only interactive so no need for static
include_graphics("files/tour.jpg", auto_pdf=F)
```

```{r tourbrush, fig.cap="A projection from a guided tour with persistent brushing applied to identify the sex and species of the ."}
# Replace with crosstalk + plotly only interactive so no need for static
